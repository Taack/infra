...
class BarDiagramScene extends RectBackgroundDiagramScene {
    ...
    void drawVerticalBackgroundAndDataBar() {
        ...
        for (int i = 0; i < xLabelList.size() / showGapEveryX; i++) {
            ...
            if (isStacked) {
                render.renderGroup(["element-type": ElementType.DATA_GROUP, startY: barY])
            } else {
                render.renderGroup(["element-type": ElementType.DATA_GROUP, startX: barX - gapHorizontalPadding, gapWidth: gapWidth, maxShapeWidth: MAX_BAR_WIDTH])
            }
            for (int j = 0; j < keys.size(); j++) {
