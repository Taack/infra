...
final class TaackJdbcService {
    ...
    final TaackResultSetOuterClass.TaackResultSet protoFromTranslator(TQLTranslator translator, Integer maxRow, int offset) {
        def b = TaackResultSetOuterClass.TaackResultSet.newBuilder()
        b.offset = offset
...
//                    def fi = new FieldInfo(new FieldConstraint(new FieldConstraint.Constraints('', false, false, null, null), null, null), 'id', (BigDecimal) 0)
        ...
        for (def row in itRow) {
            ...
            if (row instanceof String) { // only 1 column
                TaackResultSetOuterClass.Cell.Builder c = TaackResultSetOuterClass.Cell.newBuilder()
                ...
                for (def cell : row) {
                    ...
                    TaackResultSetOuterClass.Cell.Builder c = TaackResultSetOuterClass.Cell.newBuilder()
                    TaackResultSetOuterClass.Column col = itColList.next()
...
//                        c.setBytesValue()
    ...
    final private TaackResultSetOuterClass.TaackResultSet protoFromTql(String tql, Integer max, int offset) {
        protoFromTranslator(translatorFromTql(tql), max, offset)
    }

    final private static void addTableToResultSet(TaackResultSetOuterClass.TaackResultSet.Builder b, String name) {
    ...
    final private static void addColumnToResultSet(TaackResultSetOuterClass.TaackResultSet.Builder b, String name, FieldInfo col, int ordinal) {
        b.addCells(TaackResultSetOuterClass.Cell.newBuilder().setStringValue('default'))
        b.addCells(TaackResultSetOuterClass.Cell.newBuilder().setStringValue('public'))
        b.addCells(TaackResultSetOuterClass.Cell.newBuilder().setStringValue(name))
